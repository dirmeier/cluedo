...
export default class Board {
  ...
  _initPlaces(): Map<string, Place> {
    const places: Map<string, Place> = new Map();
    for (const leg of legend) {
      const pl: Place = new Place(leg.place, leg.legend, leg.type, leg.path);
  ...
  _initBoard(): Tile[][] {
    ...
    for (let i = 0; i < board.length; i++) {
      for (let j = 0; j < board[i].length; j++) {
        if (i > 0) adj[i][j].neighbors.set("up", adj[i - 1][j] || null);
        if (i < board.length - 1)
          adj[i][j].neighbors.set("down", adj[i + 1][j] || null);
        if (j > 0) adj[i][j].neighbors.set("left", adj[i][j - 1] || null);
  ...
  _distributeSuspectsToRooms(): void {
  ...
  _distributeWeaponsToRooms(): void {
    let places = this._getPlaces();
  ...
  _getPlaces(): Array<Place> {
    const els = $.filter([...this._places.values()], (i) => i.isPlace);
    return els;
  }

  putOnRandomTile(piece: Item, place: Place): Tile {
    const tile = this.getFreeTile(place);
    piece.putOn(tile);
    return tile;
  }

  getFreeTile(place: Place): Tile {
  ...
  getPiece(name: string): Item | null {
  ...
  computePath(src: Tile, target: Tile): Array<Tile> {
  ...
  computeNeighbors(distance: number, tile: Tile): Array<Tile> {
