...
export default class AI extends Player {
    ...
    addSeenCard(card) {
        ...
        if (cons === "Suspect")
            ...
            this._hasSeenPlaces.push(card.name);
    }
    wantToCast() {
        if (!this.isInPlace)
            return true;
        const hasSeenRoom = this._hasSeenPlaces.includes(this.suspect.tile.place.name);
        if (!hasSeenRoom)
    ...
    computeDestination(pips) {
        if (this._target === null) {
            const hasNotSeenPlaces = utl.distinct(this._allPlaces, this._hasSeenPlaces);
            const randomPlaceString = utl.randomElement(hasNotSeenPlaces);
            const randomPlace = Object.values(this._board.places).filter((i) => i.name === randomPlaceString);
    ...
    suggest() {
        const weaps = utl.randomElement(utl.distinct(this._allWeapons, this._hasSeenWeapons));
        const susps = utl.randomElement(utl.distinct(this._allSuspects, this._hasSeenSuspects));
        return {
            weapon: weaps,
            suspect: susps,
        };
