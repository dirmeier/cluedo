...
export default class View {
    ...
    _initAthena() {
        ...
        this._newButton(d3.select("#" + this._athenaDiv), this._legendNextButton, "Next").attr("class", "nes-btn hide-when-small");
    }
    _showText(id, text) {
    ...
    _path(el, rect, tile) {
        ...
        if (tile.isOtherRoomAndNoGate(tile.neighbors.get("down"))) {
            el.append("polyline").attr("points", x + ",  " + (h - 2) + " " + (x + w) + "," + (h - 2));
        }
    ...
    _initLegend() {
        this._initLegendForPieces("Players", this._model.players.map(function (i) {
            return i.suspect;
        }));
        this._initLegendForPieces("Suspects", this._board.suspects.sort());
        this._initLegendForPieces("Weapons", this._board.weapons.sort());
        this._initLegendForPieces("Places", Object.values(this._board.places).sort());
    ...
    _initInfo() {
        ...
        this._initSelectLists("Suspects: ", this._suspectsSelectDiv, this._suspectsSelectList, this._model.cards.suspects);
        this._initSelectLists("Weapons: ", this._weaponsSelectDiv, this._weaponsSelectList, this._model.cards.weapons);
        this._initSelectLists("Places:", this._placesSelectDiv, this._placeSelectList, this._model.cards.places);
        div = d3.select("#" + this._infoHeader).append("div");
    ...
    makeMove(oldTile, tile, path) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let mv of path) {
                yield this._drawPiece(mv, this._model.currentPlayer.suspect);
                yield this._removePiece(lastTile);
                lastTile = mv;
                yield this._paintTile(lastTile.x, lastTile.y, "lightgray");
                yield this.wait(500);
            }
        });
    }
    ...
    removePlayerFromLegend() {
        this._initLegendPieceList("legend_Players", this._model.players.map(function (i) {
            return i.suspect;
        }));
    }
    nextPlayer() {
    ...
    aiFinishMove() {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.appendInfo(`${this._model.currentPlayer.name} finishes his turn'.`);
            yield this.showFinishButton();
        });
    }
    aiMove(player) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    aiCast(player) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.appendInfo(player.name + " wants to cast a die.");
            yield this.wait(2000);
            this.appendInfo(`${player.name} decides to move ` + `to place '${player.target.name}'.`);
            yield this.wait(2000);
        });
    }
    aiAccuse(player, acc) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.wait(2000);
            this.appendInfo(`${player.name} accuses '${acc.suspect}' who he thinks committed
          the murder in '${acc.place}' with '${acc.weapon}'.`);
            yield this.wait(2000);
        });
    }
    aiSuggest(player, sugg) {
        return __awaiter(this, void 0, void 0, function* () {
            this.appendInfo(`${player.name} wants to make a suggestion.`);
            yield this.wait(2000);
            this.appendInfo(`${player.name} believes '${sugg.suspect}'
